...
number_in_letters_regex: str = "|".join(corresponding_values.keys())
first_last_nb_regex: str = rf".*?(?P<first>\d|({number_in_letters_regex}))(.*(?P<last>\d|({number_in_letters_regex})).*)?"


def convert_to_number_if_needed(number: int | str) -> int:
    try:
        return int(number)
    except ValueError:
        return corresponding_values[number]


results = []
for line in lines:
    re_match = re.search(pattern=first_last_nb_regex, string=line)
    first_number = re_match.group("first")
    last_number = re_match.group("last")

    if not last_number:
        last_number = first_number

    first_number = convert_to_number_if_needed(first_number)
    last_number = convert_to_number_if_needed(last_number)

    results.append(int(f"{first_number}{last_number}"))
